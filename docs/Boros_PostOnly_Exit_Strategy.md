...
| 机制 | 说明 | 你的用法 |
|------|------|----------|
| **ALO** (Add Liquidity Only) | 严格 Post-Only，如果会立即成交则**整笔回滚** | 挂单必用，绝不做 Taker |
| **SOFT_ALO** | 宽松 Post-Only，跳过可成交部分，剩余挂单 | ALO 的柔性替代 |
| **Stop Loss (Market)** | 链下条件单，APR 触及阈值 → 自动 IOC 市价平仓 | 被吃后预设止损 |
| **AMM 路由** (`includeAmm=true`) | 走 AMM 获取即时流动性，比干等订单簿快 | 流动性枯竭时快速退出 |
| **bulkOrders** | 一笔交易完成「撤旧单 + 挂新单」 | 平仓时原子操作 |

---

## 二、核心问题分析：为什么会被吃？

你的限价单被对手方吃掉 ≠ 运气不好。**被吃的原因决定了你应该怎么平仓：**

```
被吃原因               信号特征                    最优平仓策略
─────────────────────────────────────────────────────────────
🔴 逆向选择 (Adversarial)   成交价偏离 mid > 2x spread   → 立即 IOC 市价跑
   有人知道利率要往哪走       成交后利率持续单向移动          越快越好！
   
🟡 波动率尖峰 (Vol Spike)   波动率突然 > 3x 均值          → 阶梯限价等均值回复
   市场噪音/算法冲击         利率大概率会回归               不要追涨杀跌

🟢 Spread 捕获 (Normal)     正常成交, 偏离小             → 混合: 一半限价等 + 一半止损
   你的报价刚好是市场最优       无信息劣势                    不急, 优化执行价格

⚪ 流动性枯竭 (Liq Vacuum)  价差 > 50 bps                → 走 AMM 即时退出
   订单簿几乎没对手方         你的挂单是唯一流动性              别跟空气交易
```
...
```
                    平仓速度    预估成本(bps)    适用场景
MARKET_IMMEDIATE     ⚡即刻       29.5          逆向选择确定
LIMIT_ESCALATOR      🐢较慢        9.0          波动尖峰 (可能回复)
HYBRID (推荐)        🕐平衡        2.3          正常被吃
AMM_ROUTE            ⚡即刻       22.5          流动性枯竭
STOP_LOSS            ⚡自动       中等          预设保底线
```

### 1. MARKET_IMMEDIATE — 立刻跑

被逆向选择吃掉时用。直接 IOC 市价单平仓，走 AMM+订单簿双通道。

```json
// Boros calldata: 反向 IOC 单, includeAmm=true
{
    "side": "LONG",         // 你变 SHORT → 需要 LONG 平仓
    "size": <full_position>,
    "tif": "IOC",
    "includeAmm": true      // 走 AMM 吃即时流动性
}
```

**成本**：付出全价差（约 29.5 bps），但阻止了更大的方向性亏损。

### 2. LIMIT_ESCALATOR — 阶梯撤退

波动率尖峰时用。分 3 档挂限价单，从宽松到激进：

- 第 1 档：接近 mid，等均值回复（如果回到你的价格，零成本平仓）
- 第 2 档：稍激进，中等价格
- 第 3 档：很激进（接近 market），确保一定能出去

```
档位    利率                    激进程度     容量
L1      10.53% (接近 mid)       宽松         1/3 仓位
L2      10.58%                  中等         1/3 仓位
L3      10.62% (接近 market)    激进         1/3 仓位
```

**核心思想**：时间是你的朋友。波动率尖峰后，50% 概率利率会回调。如果回调到 L1，你以零额外成本平仓；如果不回调，L3 兜底。

### 3. HYBRID（推荐默认）— 一半等 + 一半止损

最平衡的策略：
- **50% 仓位**：挂 GTC 限价单在最优平仓价
- **50% 仓位**：预设 Boros Stop Loss 条件单（APR 偏离 30 bps 自动触发）

```json
// 限价单
{"side": "LONG", "tick": <optimal_exit_tick>, "size": half, "tif": "GTC"}

// 止损条件单 (Boros Stop Order Service)
{
    "stopAprOrderType": 3,    // Stop Loss (Market)
    "tick": <trigger_tick>,   // 偏离 30 bps 时触发
    "size": half,
    "timeInForce": 1          // IOC
}
```
...
订单簿流动性枯竭时，`includeAmm=true` 让 IOC 单直接匹配 AMM 流动性池。AMM 提供连续报价，不受 tick 离散限制。

### 5. STOP_LOSS — 预设保险

利用 Boros 的 **Stop Order Service**（链下条件单），设定「如果 APR 再偏离 X bps → 自动市价平仓」。设置后不需要监控，适合睡觉时保护仓位。

---

## 四、防止被吃的第一道防线：自适应 Spread

最好的平仓是**不需要平仓**。当市场波动增大时，主动扩大挂单价差：
...
| 波动增大 | 3× | 0% | **21 bps** |
| 剧烈波动+满仓 | 5× | 80% | **70 bps** |
| 极端+满仓 | 10× | 100% | **114 bps** |

```
spread = base_spread × sqrt(recent_vol / avg_vol) × (1 + 2 × position_ratio)
```

价差越大 → 你的挂单离 mid 越远 → 被吃的概率越低 → 吃到时利润越大（覆盖你被吃后的平仓成本）。
...
```
        ...
        ┌───────────────────────┼───────────────────────┐
        ▼                       ▼                       ▼
   ADVERSARIAL            VOL_SPIKE              SPREAD_CAPTURE
        │                       │                       │
        ▼                       ▼                       ▼
   IOC 市价全平           3档阶梯限价               HYBRID 混合
   includeAmm=True        等均值回复            50%限价+50%止损
```
...
完整实现：`strategy/post_only_exit.py`

核心类 `PostOnlyExitEngine` 提供：
- `generate_post_only_quotes()` — 生成 ALO 纯挂单
- `classify_fill()` — 分类被吃原因
- `generate_exit_plan()` — 生成平仓计划（订单+止损）
- `adaptive_spread()` — 自适应价差调宽
- `optimal_exit_rate()` — 最优平仓利率计算

---

## 附：Python 源码 (post_only_exit.py)
